...
export default function HomePage() {
  ...
  // LOGIKA LIMIT (Menggunakan Memori Permanen Browser)
  const handleCardClick = (e: React.MouseEvent, url: string) => {
    ...
    console.log("Klik Ke:", currentCount + 1);
    ...
    if (currentCount >= 1) {
      ...
      if (typeof window !== "undefined") {
        localStorage.setItem("guidify_clicks", "1"); // Catat sudah pakai 1
      }
      window.open(url, '_blank'); // Buka link
    }
  };

  return (
    // PERUBAHAN PENTING: Kita pakai Fragment (<>...</>) 
    // agar Navbar, Main, dan Modal sejajar (tidak saling menjepit)
    <>
      <Navbar />
      ...
      {/* Ini memastikan modal punya 'kekuasaan' penuh untuk menutupi layar */}
      <PricingModal 
        isOpen={isModalOpen} 
